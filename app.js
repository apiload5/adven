...
// --- UPDATED PARSER WITH STRICT: FALSE FOR CORRUPT FEEDS ---
...
// --- MODIFIED FUNCTION FOR UNIQUE, TRUTHFUL, AND TOKEN-SAVING REWRITE (H1 REMOVED) ---
...
// ... (generateImageAlt, generateImageTitle, generateTags, createBloggerPost functions are unchanged) ...
...
async function processOnce() {
  try {
    ...
    for (const item of items) {
      ...
      // === Nayi Safai (Cleanup) Lines ===
      if (fullContent) {
          // 1. Image tags hatao jo content mein pehle se ho sakti hain
          fullContent = fullContent.replace(/<img[^>]*>/gi, ''); 
          // 2. Line breaks/corrupt tags ko saaf karo (XML errors ko kam karega)
